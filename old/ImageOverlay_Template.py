...
class ImageOverlayApp:
    ...
    def setup_buttons_window(self):
        ...
        btn_increase_buttons_transparency = tk.Button(
            btn_frame, text="Btns Transp+", command=self.increase_buttons_transparency)
        btn_increase_buttons_transparency.grid(row=0, column=0, pady=5, sticky='ew')

        btn_decrease_buttons_transparency = tk.Button(
            btn_frame, text="Btns Transp-", command=self.decrease_buttons_transparency)
        ...
        self.btn_toggle_transparency = tk.Button(
            btn_frame, text="Toggle Transp", command=self.toggle_transparency)
        ...
        btn_load_image = tk.Button(
            btn_frame, text="Load 1st Img", command=lambda: self.load_image("First Image"))
        btn_load_image.grid(row=load_hide_row, column=0, pady=5, sticky='ew')

        btn_load_second_image = tk.Button(
            btn_frame, text="Add 2nd Img", command=lambda: self.load_image("Second Image"))
        btn_load_second_image.grid(row=load_hide_row, column=1, pady=5, sticky='ew')

        self.btn_hide_show_image = tk.Button(
            btn_frame, text="Hide Window", command=self.toggle_image_window)
        ...
        btn_flip_horizontal = tk.Button(
            btn_frame, text="Flip H", command=self.flip_image_horizontal)
        btn_flip_horizontal.grid(row=flip_row, column=0, pady=5, sticky='ew')

        btn_flip_vertical = tk.Button(
            btn_frame, text="Flip V", command=self.flip_image_vertical)
        ...
        self.btn_set_rotation_point = tk.Button(
            btn_frame, text="Set Rot Pt", command=self.toggle_rotation_point_mode)
        ...
        # Fine Zoom Level buttons
        fine_zoom_buttons_row = zoom_buttons_row + 1
        btn_fine_zoom_in = tk.Button(
            btn_frame, text="+ Fine", command=self.fine_zoom_in)
        btn_fine_zoom_in.grid(row=fine_zoom_buttons_row, column=0, pady=5, sticky='ew')

        btn_fine_zoom_out = tk.Button(
            btn_frame, text="- Fine", command=self.fine_zoom_out)
        btn_fine_zoom_out.grid(row=fine_zoom_buttons_row, column=1, pady=5, sticky='ew')

        # Fine Rotation buttons
        fine_rotation_buttons_row = fine_zoom_buttons_row + 1
        btn_fine_rotate_clockwise = tk.Button(
            btn_frame, text="Rot +0.5°", command=self.fine_rotate_clockwise)
        btn_fine_rotate_clockwise.grid(row=fine_rotation_buttons_row, column=0, pady=5, sticky='ew')

        btn_fine_rotate_counterclockwise = tk.Button(
            btn_frame, text="Rot -0.5°", command=self.fine_rotate_counterclockwise)
        btn_fine_rotate_counterclockwise.grid(row=fine_rotation_buttons_row, column=1, pady=5, sticky='ew')

        # Undo/Redo buttons
        undo_redo_row = fine_rotation_buttons_row + 1
        self.btn_undo_move = tk.Button(
            btn_frame, text="Undo", command=self.undo_move, state='disabled')
        self.btn_undo_move.grid(row=undo_redo_row, column=0, pady=5, sticky='ew')

        self.btn_redo_move = tk.Button(
            btn_frame, text="Redo", command=self.redo_move, state='disabled')
        ...
        tk.Label(btn_frame, text="Active Image:").grid(row=active_image_row, column=0, pady=5, sticky='e')
        ...
        self.chk_first_image_visibility = tk.Checkbutton(
            btn_frame, text="Show 1st", variable=self.first_image_visibility_var,
        ...
        self.chk_second_image_visibility = tk.Checkbutton(
            btn_frame, text="Show 2nd", variable=self.second_image_visibility_var,
    ...
    def bind_canvas_events(self):
        ...
        # Keyboard events can be added here if needed
    ...
    def on_right_click(self, event=None):
        ...
        active_image.offset_x = 512
        active_image.offset_y = 512
