...
class ImageOverlayApp:
    ...
    def __init__(self, root):
        ...
        self.load_default_second_image()
    ...
    def create_brainstorm_window_button(self, parent):
        ...
        self.btn_toggle_brainstorm_window.grid(row=12, column=0, columnspan=2, pady=5, sticky='ew')
    ...
    def load_default_second_image(self):
        """
        Loads 'liniar.svg' as the second image by default, from the 'Images' subdirectory.
        """
        script_dir = os.path.dirname(os.path.abspath(__file__))
        images_dir = os.path.join(script_dir, 'Images')
        filepath = os.path.join(images_dir, 'liniar.svg')
        if os.path.exists(filepath):
            image_original = self.open_image_file(filepath)
            if image_original:
                image_state = ImageState(image_original, "Second Image")
                self.images["Second Image"] = image_state
                self.active_image_name = "Second Image"
                self.update_active_image_menu()
                self.active_image_var.set("Second Image")
                self.draw_images()
        else:
            print(f"'liniar.svg' not found in {images_dir}")
    ...
    def open_brainstorm_window(self):
        ...
        new_window.geometry("800x600")  # Increased width to accommodate toolbar
    ...
    def close_brainstorm_window(self):
        """
        Closes the brainstorm window and updates the button text.
        """
        if self.brainstorm_window:
    ...
    def load_image_brainstorm_window(self, window, filepath=None):
        ...
        if filepath:
            ...
            if image_original:
                # Get the canvas
                canvas = window.children['!frame'].children['!canvas']
    ...
    def save_brainstorm_image(self, window):
        """
        ...
        if canvas.image_pil:
            ...
            if filepath:
                try:
                    ...
                    tk.messagebox.showinfo("Image Saved", f"Image saved successfully to:\n{filepath}")
                except Exception as e:
                    tk.messagebox.showerror("Save Error", f"Error saving image:\n{e}")
        else:
            tk.messagebox.showwarning("No Image", "There is no image to save.")
